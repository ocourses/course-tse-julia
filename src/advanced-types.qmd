...
We can see the term *promote* in the output. This means the compiler decided to convert the input integer to a floating point number in order to compute the sum of two `Float64`. This choice has nothing to do with the typing hierarchy seens earlier in this lecture. The true reason is that casting a `Float64` to an `Int64` is ambiguous. Indeed, rounding down `2.5` produces the integer `2` and rounding up the integer `3`. The sum `1+2` and `1+3` does, anyway, not produce the expected result `1+2.5=3.5`.
