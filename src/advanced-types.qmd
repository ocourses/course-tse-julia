...
*(Ref. Chris Rackauckas)*

Common knowledge has it that Julia is fast because it is JIT compiled. The actual reason is the *type inference* algorithm executed before the compiler combined with *type specialization* in functions. 
...
This is straight forward if the instruction has the type information. An 64 bit integer and floating point number require the same memory space. However, this memory chunck is interpreted differently. The binary representation of the integer is `[sign bit][value in two's complement on 63 bits]` and `[sign bit][11 exponent bits][52 mantissa bits]` for a floating point number. This is what happens with C or Fortran code. 

The type should be quired when unknown by the instruction. Then the associated set of operations for this type should be loaded. This is what happens with Python code. 
...
Let us have a look at what can be done from a Julia *software* programming point of view. Providing hints to the type inference algorithm allows us to maximize fast executing CPU instructions. The global scope of the REPL is too dynamic to be optimized. 
...
The type of `a` and `b` was found here by human knowledge. The type inference algorithm would have associated `Any` to both `a` and `b`. Since the type of those variables could asynchronously change within the REPL, further in this section type inference in functions is only presented.
...
At first glance, we have written one single function. Actually, different functions will be used depending on the type of the input values. This can be seen by looking at the generated CPU instructions for different executions. To be more precice, we will look at the intermediate representation (IR) generated by the LLVM Julia compiler. 
...
::: {.callout-caution collapse="true" icon=false}
## How do the input types impact the output type of `f`?
...
Let us have a look at what happens when we mix types in the function call.

```{julia}
@code_llvm f(1,2.5)
```

We can see the term *promote* in the output. This means the compiler decided to convert the input integer to a floating point number in order to compute the sum of two `Float64`. This choice has nothing to do with the typing hierarchy seens earlier in this lecture. The true reason is that casting a `Float64` to an `Int64` is ambiguous. 

The following function breaks type stability. 
...
This is illustrated using `@code_warntype`.
...
The type of the output of `h`, either `Int64` or `Float64`, does not depend on the type of the input parameters but on the value of their sum `x + y`. This information can not be predicted at compile time. The compiler can only restrict the output to `Union{Float64, Int64}`.
:::

This leads us to the following general rule. 
