...
To display output in Julia, use the `println` function:
...
Julia functions allow you to group code into reusable components. Here’s an overview of different ways to define functions, use arguments, set default values, and handle mutation.

### Function Definitions
Julia provides two main ways to define functions: the classic form and the assignment form.

**Classic Form**: Functions are defined using the `function` keyword and closed with `end`.
...
**Assignment Form**: Functions can also be defined in a single line with the assignment operator `=`.

```{julia}
add(a, b) = a + b
```

Both definitions above create a function `add` that takes two arguments and returns their sum.

### Arguments and Keyword Arguments
In Julia, functions can take positional arguments, keyword arguments, and a variable number of arguments.

- **Positional Arguments**: These are specified in the function’s parameter list and must be passed in the correct order.

- **Keyword Arguments**: Specified with a semicolon (`;`), these arguments are passed by name and have default values. 
...
- **Variable Number of Arguments**: To accept an arbitrary number of arguments, use the splatting operator `...`. These arguments are collected into a tuple.
...
In Julia, arguments can be given default values by specifying them in the function definition. If no value is provided when the function is called, the default is used.
...
In Julia, functions that modify or mutate their arguments typically have a `!` at the end of their name, following the "bang" convention. This is not enforced by the language but is a widely followed convention in Julia.
...
arr = [1, 2, 3]
add_one!(arr)
println(arr)  # Outputs: [2, 3, 4]
```

In this example, `add_one!` modifies the elements of the array `arr`. By convention, the `!` indicates that the function mutates its input.

### Return Values
In Julia, a function returns the last evaluated expression by default. Alternatively, you can use the `return` keyword to explicitly specify the output.
...
This covers the basics of defining functions, handling different types of arguments, using default values, and following Julia's conventions for mutation.

## Scoping and Closure in Julia
...
println(access_global())  # Outputs: 10
```
...
println(closure(4))  # Outputs: 9 (3 + 2 + 4)
```
...
### Closures
A closure is a function that captures the variables from its surrounding lexical scope. Closures are particularly useful for creating function factories or when working with callbacks.
...
In this case, `make_multiplier` returns a function that captures the `factor` variable, allowing the returned function to use it later.
...
function increment_global()
    global_counter += 1  # Modifying a global variable
end

# Local variable
function increment_local()
    local_counter = 0
    local_counter += 1  # Only exists within this function
